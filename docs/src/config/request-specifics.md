...
As this property is optional, when not provided, all properties will inherit from the
...
the interpreter can't determine the response's TTL value to override this one.

:::

The time until the cached value expires in milliseconds.
...
::: details This can be seen when opening the network tab in your browser's dev tools.

![Network tab in Chrome DevTools](../assets/disk-cache-screenshot.png 'A network disk-cache example')
...
for `POST`, `PUT`, `DELETE` and other methods, as the default is only `GET`.
...
- Default:
  ...
  _(These default status codes follow RFC 7231)_

An object or function that will be tested against the response to indicate if it can be
cached. You can use `statusCheck`, `containsHeaders`, `ignoreUrls`, `allowUrls` and
...
axios.get<{ auth: { status: string } }>('url', {
  cache: {
    cachePredicate: {
      ...
      // only cache request URLs that include "weekly"
      allowUrls: ['weekly']
    }
  }
});
  ...
  { username, password },
  {
    cache: {
      update: {
        ...
        // An example that updates the "user info response cache" when doing a login.
        ...
        [userInfoResponseId]: (cachedValue, response) => {
          ...
          // This returned value will be returned on subsequent calls to the cache.
          return cachedValue;
        }
      }
    }
...
To use `true` (automatic ETag handling), the `interpretHeader` option must be set to `true`.
...
header is sent to us, then the value from that header is used; otherwise, the cache creation
...
Enables the cache to be returned if the request fails because of an invalid status code, a
network error, etc. You can filter the type of error that should be stale by
using a predicate function.

::: warning

If the response is treated as an error because of an invalid status code _(like when using [statusCheck](#cache-cachepredicate))_, and this ends up `true`, the cache will be preserved over the "invalid" request.
...
const customPredicate = (response, cache, error) => {
  // Blocks staleIfError if it has a response
  return !response;
...
This option bypasses the current cache and always makes a new HTTP request. This will not
...
- `string[]`: Overrides the server-sent Vary value and uses specific request headers in cache key
...
Asynchronously called when a network request is needed to resolve the data, but an older and probably expired cache entry exists. Calls it with the current data **BEFORE** the network request starts, so you can use it to temporarily update your UI with expired data before the network returns.
...
If the Axios call returns cached data, meaning no network will be involved, the
