...
export interface CacheProperties<R = unknown, D = unknown> {
   ...
   *
   * If a function is used, it will receive the complete response and is expected to return
   * a TTL value.
   *
   * When using `interpretHeader: true`, this value will only be used if the interpreter
   * can't determine its TTL value to override this
   *
  ...
  /**
   * As most of our cache strategies depend on well-defined HTTP headers, most
   * browsers also use those headers to define their own cache strategies and storages.
   *
   * When your requested routes include `Cache-Control` in their responses, both the
   * library and your browser may cache the response, resulting in a **double layer of
   * cache**.
   *
   ...
   * header is sent to us, then the value from that header is used, otherwise cache creation
  ...
  /**
   * Enables cache to be returned if the response comes with an error due to an invalid
   * status code, network errors, etc. You can filter the type of error that should be
  ...
  /**
   * This option bypasses the current cache and always makes a new HTTP request. This will
   * not delete the current cache, it will just replace the cache when the response
   * arrives.
   *
   * Unlike `cache: false`, this will not disable the cache, it will just ignore the
   ...
   * older one **and probably expired** cache exists. It is called with the current data **BEFORE**
...
export interface CacheInstance {
   ...
   * that prioritizes the id, and if not specified, a string is generated using the
   * `method`, `baseURL`, `params`, `data` and `url`.
   *
   * You can learn how to use it on the [Request
   ...
   * as some sort of listener to know when a request is waiting for another to finish.
   *
   * @deprecated This property will be hidden in future versions. Please tell us why you need it at https://github.com/arthurfiorette/axios-cache-interceptor/issues/1158
   * @default new Map()
  ...
  /**
   * The function that will be used to intercept the response after it is returned by the
   * axios adapter.
   *
   ...
   * function that receives a string and returns nothing.
   *
...
/**
 * An object of any type that can be used to log debugging information in
