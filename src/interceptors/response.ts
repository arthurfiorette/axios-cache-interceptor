...
import type { CacheResponseInterceptor } from './build';
  ...
  return {
    async onFulfilled(response) {
      const id =
        (response.id =
        response.config.id ??=
          axios.generateKey(response.config));
    ...
    async onRejected(error) {
      const config = error.config as FullCacheRequestConfig;
      ...
      if (cacheConfig?.staleIfError) {
        ...
        if (
          ...
          (typeof staleIfError === 'number' &&
            cache.createdAt + staleIfError > Date.now())
        ) {
          // Resolve all other requests waiting for this response
