...
describe('Interceptor Utilities', () => {
  ...
  describe('updateStaleRequest', () => {
    it('should add If-None-Match header when etag is true and cache has etag', () => {
      const cache: StaleStorageValue = {
        data: {
          headers: { [Header.ETag]: 'abc123' }
        } as any,
        createdAt: Date.now()
      };
      ...
      expect(config.headers?.[Header.IfNoneMatch]).toBe('abc123');
    });

    it('should add If-None-Match header when etag is a string', () => {
      const cache: StaleStorageValue = {
        data: {
          headers: { [Header.ETag]: 'original-etag' }
        } as any,
        createdAt: Date.now()
      };
      ...
      const config: ConfigWithCache<any> = {
        headers: {},
        cache: { etag: 'custom-etag' }
      };
      
      updateStaleRequest(cache, config);
      
      expect(config.headers?.[Header.IfNoneMatch]).toBe('custom-etag');
    });

    it('should add If-Modified-Since header when modifiedSince is true and cache has last modified', () => {
      const lastModifiedTime = 'Wed, 21 Oct 2020 07:28:00 GMT';
      const cache: StaleStorageValue = {
        data: {
          headers: { [Header.LastModified]: lastModifiedTime }
        } as any,
        createdAt: Date.now()
      };
      ...
      expect(config.headers?.[Header.IfModifiedSince]).toBe(lastModifiedTime);
    });

    it('should add If-Modified-Since header using createdAt when last modified is not in cache', () => {
      const createdAt = Date.now();
      const expectedDate = new Date(createdAt).toUTCString();
      ...
      expect(config.headers?.[Header.IfModifiedSince]).toBe(expectedDate);
    });

    it('should add If-Modified-Since header when modifiedSince is a Date object', () => {
      const customDate = new Date('2022-01-01T00:00:00Z');
      const customDateString = customDate.toUTCString();
      
      const cache: StaleStorageValue = {
        data: {
          headers: { [Header.LastModified]: 'old-date' }
        } as any,
        createdAt: Date.now()
      };
      ...
      const config: ConfigWithCache<any> = {
        headers: {},
        cache: { modifiedSince: customDate }
      };
      
      updateStaleRequest(cache, config);
      
      expect(config.headers?.[Header.IfModifiedSince]).toBe(customDateString);
    });

    it('should initialize headers if they are undefined', () => {
      const cache: StaleStorageValue = {
        data: {
      ...
      const config: ConfigWithCache<any> = {
        cache: { etag: true }
      }; // No headers initially
      
      updateStaleRequest(cache, config);
      
      expect(config.headers).toBeDefined();
      expect(config.headers?.[Header.IfNoneMatch]).toBe('test-etag');
    });

    it('should handle when both etag and modifiedSince are set', () => {
      const customDate = new Date('2022-01-01T00:00:00Z');
      const customDateString = customDate.toUTCString();
      
      const cache: StaleStorageValue = {
        data: {
          headers: { [Header.ETag]: 'test-etag', [Header.LastModified]: 'old-date' }
        } as any,
        createdAt: Date.now()
      };
      ...
      const config: ConfigWithCache<any> = {
        headers: {},
        cache: { etag: true, modifiedSince: customDate }
      };
